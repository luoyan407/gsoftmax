...
function DataLoader:run()
   ...
   local function enqueue()
      while idx <= size and threads:acceptsjob() do
         ...
         threads:addjob(
            function(indices, nCrops, cpuType, split, featSize)
               local sz = indices:size(1)
               local batch, imageSize, salSize, imgnames
               local target
