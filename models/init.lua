...
local function ConvInit(clayer)
   local n = clayer.kW*clayer.kH*clayer.nOutputPlane
   clayer.weight:normal(0,math.sqrt(2/n))
...
function M.setup(opt, checkpoint)
   ...
   elseif opt.testOnly then
      local pretrinedModel = torch.load(opt.retrain)
      if #pretrinedModel == 0 then
         model = pretrinedModel.model:type(opt.tensorType)
         criterion = pretrinedModel.criterion:type(opt.tensorType)
      else
         model = pretrinedModel:type(opt.tensorType)
      end
   elseif opt.iFunc == 'finetune' then
      model = torch.load(opt.retrain)
      local lastDepth = model:get(#model).weight:size(2)
      model:remove(#model)
      model:remove(#model)
      model:remove(#model)

      local flcnt = nn.Linear(lastDepth, opt.nClasses)
      flcnt.bias:zero()

      model:add(nn.SpatialMaxPooling(opt.featSize,opt.featSize,1,1))
            :add(nn.View(-1,lastDepth))
            :add(flcnt)
