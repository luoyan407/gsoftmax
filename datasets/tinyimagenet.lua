...
function ImagenetDataset:__init(imageInfo, opt, split)
   ...
   self.targetSize = opt.imgSize -- 224
...
function ImagenetDataset:get(i)
   ...
   local filename = string.sub(path,string.find(path, '/', 1, true)+1,string.len(path))
...
function ImagenetDataset:preprocess()
   if self.split == 'train' then
      return t.Compose{
         t.RandomSizedCrop(self.targetSize),
         t.ColorJitter({
            brightness = 0.4,
            contrast = 0.4,
            saturation = 0.4,
   ...
   elseif self.split == 'val' then
      ...
      return t.Compose{
         t.Scale(self.targetSize),
         t.ColorNormalize(meanstd),
         Crop(self.targetSize),
      }
      --t.Scale(256),
   else
